...
const inputId = document.getElementById("inputId");
    // change는 작성 후 벗어나면 검사
inputId.addEventListener("change", function(){

    const regExp = /^[a-z][\w!@#$%^&*_-]{5,13}$/; 
    // 소문자 시작(1) + 나머지(5 ~ 13) 글자 = 6 ~14 글자 
...
inputPw.addEventListener("keyup", function(){ // 비밀번호란에 작성했을 때 검사
...
inputPwConfirm.addEventListener("keyup", function(){ // 비밀번호 확인란에 작성했을 때 검사
...
// 만약 하나라도 false다  -> 유효성 검사가 완료되지 않았습니다.
function validate() {

    for(let key in checkObj) {
        if( !checkObj[key] ) { // false일 때 띄어주기
