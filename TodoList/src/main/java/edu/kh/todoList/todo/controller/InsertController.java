...
public class InsertController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			Member member = (Member)session.getAttribute("loginMember"); // session에 올라가는 건 object로 올라감 => member로 다운캐스팅 필요
			...
			if(result > 0) {
				...
				resp.sendRedirect(req.getHeader("referer")); // 이전 페이지로 돌아가기
