...
public class ExampleController2 extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// req에 요청받은 데이터가 들어가 있다
		
		// 원래 tomcat9 버전때는
		// 전달받은 값을 얻어와서 확인 -> 한글이 깨지는 문제가 발생!
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/result.jsp"); // JSP 한테 Servlet이 어떤 요청 받았는지 알려줌
																// webapp 폴더 기준 
		dispatcher.forward(req, resp); 
		// 클라이언트 요청 -> Servlet이 받음 
		// forward : 요청에 따라 JSP에 위임하는 것
			// 요청된 페이지가 주소창에 뜸
