...
//        아니면 "아이디 또는 비밀번호가 일치하지 않습니다" 출력
		...
		= req.getRequestDispatcher("/WEB-INF/views/loginResult.jsp"); // webapp 폴더 기준
		
		
		// message값을 속성 (attribute)으로 추가
		// -> Map 형식으로 작성 (K:V)
		req.setAttribute("message", message); // 속성을 세팅해줌 key값 value (message라는 이름으로 구해둔 message 변수 넘김)
