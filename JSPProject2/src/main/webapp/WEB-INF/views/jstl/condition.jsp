...
<body>
	...
	<c:if test="${name eq '홍길동'}"> <!-- name값이 홍길동과 일치하면 -->
		<h3>이름이 일치합니다.</h3>
	</c:if>

	<c:if test="${name ne '홍길동'}"> <!-- name값이 홍길동과 일치하지 않으면 -->
		<h3>이름이 일치하지 않습니다.</h3>
	</c:if>
	...
	<c:choose>
		<%-- 무조건 JSP용 주석(<%-- --> 사용하기 (<!-- -->(html)용 주석) 사용 금지 --%>
		<%-- 만약에 val이 100 초과한다면 ~ 띄어줄 것이다 
			 choose 안에 주석 작성이 필요하다면 반드시 JSP용 주석 사용!!!
			 --%>
		<c:when test="${param.val gt 100}"> <!-- gt 100 : 100초과 -->
			100 초과
		</c:when>
